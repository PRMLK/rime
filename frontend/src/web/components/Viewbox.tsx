...
import { mobileRouteChangeEvent } from '@/shared/mobile-route';
import './Viewbox.css';
 ...
 * MobilePlayer（移动播放器）的页面状态，因此根路径仍可作为正式入口独立使用。
 *
 * @param props.previewSource - 需要加载的预览页面地址，默认使用根路径移动端入口。
 * @returns 包含设备选择、缩放、刷新、状态条及视觉辅助工具的预览画布。
 * @example
 * <Viewbox previewSource="/" />
 */
export function Viewbox({ previewSource = import.meta.env.VITE_VIEWBOX_SRC ?? '/' }: ViewboxProps) {
